...
# python scripts/step1.py --input ${input_file} --size ${areaSize} --output intermediate1.txt
def main(argv):
    parser = argparse.ArgumentParser(description="Check the transcript denisty around a slice.")
    
    # Input
    parser.add_argument("-i", "--input", required=True, help="path to folder containing detected_transcripts.csv")
    parser.add_argument("-s", "--size", type=int, required=True, help="sample area size in uM (length of one side of the square). Smaller length = larger number of sample bins; default = 100uM")
    parser.add_argument("-o", "--output", required=True, help="path to output directory")
    
    args = parser.parse_args()
    mols = (args.input + "/detected_transcripts.csv")
    binsize = args.size
    outPath = args.output
    ...
    indices_with_counts.to_csv(outPath+'/MERFISH_QC/MoleculeDensity.csv', index=False)
    ...
    plt.savefig(outPath+"/MERFISH_QC/MoleculeDensityHeatmap.jpg", dpi=200, bbox_inches="tight", pad_inches=0)
    ...
    plt.yscale('log') 
    plt.savefig(outPath+"/MERFISH_QC/MoleculeDensityHistogram.jpg", dpi=100, bbox_inches="tight", pad_inches=0)
