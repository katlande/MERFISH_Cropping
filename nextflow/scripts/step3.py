...
# python scripts/step3.py --input ${inputPath} --output ${outPath}
def main(argv):
    ...
    parser.add_argument("-d", "--input", required=False, default='', help="MERFISH directory (directory containing cell meta data and parquet files)")
    ...
    parser.add_argument("-y", "--revy", type=bool, default=True, help="Transform the y axis of the polygon image (default: False)")
    ...
    mfpath=args.input
    img=args.img
    parq="cell_boundaries.parquet"
    met="cell_metadata.csv"
    outPath=args.output
    
    # pull
    mols = pd.read_csv(outPath+"/MoleculeDensity.csv")
    ...
    plt.savefig(outPath+'/MERFISH_QC/PolygonOverlay.jpg', dpi=outputdpi, bbox_inches="tight", pad_inches=0)
